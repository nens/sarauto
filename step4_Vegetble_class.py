...
def main():
    ...
    logger.info(selectDates)    
    logger.info(ts_listAll)
...
### ------------Built in function -------------####
def add_missing_timesteps(files_to_verify, startTS, endTS, daysInterval):
    """List a consecutive set of files as expected by the next functions.
    When the file does not exist, the missing data gets interpolated from the existing files
    """
    file_date_strings = [file.split("_")[-1].split(".")[0] for file in files_to_verify]
    file_dates = [datetime.datetime.strptime(file_date_string, "%d%b%Y") for file_date_string in file_date_strings]
    files = []
    step = timedelta(days=daysInterval)
    timestep = startTS
    counter = 0
    while timestep <= endTS:
        if timestep not in file_dates:
            counter_file = files_to_verify[counter]
            counter_file_date_str = counter_file.split("_")[-1].split(".")[0]
            missing_file = counter_file.replace(counter_file_date_str, timestep.strftime("%d%b%Y"))
            files.append(missing_file)
        else:
            files.append(files_to_verify[counter])
            counter += 1
        timestep += step
    return files
...
def filesCategorizeByDate(folder, extension, headStr, splitStr, startTSstr, daysInterval, endTSstr, time_formatStr):
    ...
    file_date_strings = [file.split("_")[-1].split(".")[0] for file in all_files]
    all_dates = [datetime.datetime.strptime(file_date_string, "%d%b%Y") for file_date_string in file_date_strings]
    files_to_verify = [file for date, file in zip(all_dates, all_files) if date >= startTS and date <= endTS]
    vv_files_to_verify = [f for f in files_to_verify if "VV" in f]
    vh_files_to_verify = [f for f in files_to_verify if "VH" in f]
    logger.info("files_to_verify filesCategorizeByDate")
    logger.info(files_to_verify)
    vv_files = add_missing_timesteps(vv_files_to_verify, startTS, endTS, daysInterval)
    vh_files = add_missing_timesteps(vh_files_to_verify, startTS, endTS, daysInterval)
    files = vv_files + vh_files

    logger.info("files filesCategorizeByDate")
    logger.info(files)
    numC = len(headStr)
    num_f = len(files)
