    ...
    if not os.path.isfile(filepath):
        ...
        if os.path.exists(dim_name):
            ...
            try:
                ...
                logger.info("GPT Stack for " + os.path.basename(dim_name) + ' is complete')
                print("GPT Stack for " + os.path.basename(dim_name) + ' is complete')
                os.remove(filepath)
            except:
                print('Failed to complete GPT Stack for ' + os.path.basename(dim_name))
                logger.error('Failed to complete GPT Stack for ' + os.path.basename(dim_name))
                sys.exit(0)